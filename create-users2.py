...
How the Dry Run works:
    The Dry Run works by asking the user if they want to run it in dry mode or not
    If so, it would simulate how the it would look like and if the information is correct based of the information give
    It would throw an Exception if something goes wrong and what was the reason, instead of giving an error or proceed with the actual creation of the user.
    If the user doesn't want to run it in Dry run mode, it would execute the actual user automation creation.
'''
...
user_input= input("Do you want to run it on Dry mode (Y/N):").strip().lower() == 'y'
    ...
    try:
        ...
        #The reason being  the # is normally comments and preventing to have error or incorrect data output  
            if match or len(fields) != 5:
                if user_input:
                    if match:
                        print(f"Comment Line: {line.strip()}")
                    else:
                        print(f"Error line (needs 5):  {line.strip()}")
                continue
